...
description: 使用少样本提示技术为用户任务撰写可直接使用的 Prompt，用户要求沉淀样例时可将输入、推理过程和输出追加到指定持久化位置。Use when the user provides a task with optional 1-5 input/output or input/reasoning/output examples, or wants a generated prompt containing role, instruction, optional examples, output format, and target input placeholder. If examples are absent, generate a zero-shot prompt with a brief notice instead of asking follow-up questions.
---
...
1. 读取用户任务、输出格式和可选示例。
2. 明确角色、任务指令、示例区、输出要求和待处理内容占位符。
3. 将用户示例写入生成的 Prompt；如果没有示例，写入零样本说明，并依赖任务说明、输出要求和约束生成 Prompt。
4. 输出”生成的少样本提示 Prompt”。
5. 只有当用户明确说要保存、沉淀、加入样例库，或 payload 设置 `persist_sample: true` 时，才把本轮”输入-推理过程-输出”追加到用户指定的持久化位置。
...
4. 如果用户或 payload 明确要求持久化样例，追加写入用户指定的持久化位置；不要覆盖已有内容。
